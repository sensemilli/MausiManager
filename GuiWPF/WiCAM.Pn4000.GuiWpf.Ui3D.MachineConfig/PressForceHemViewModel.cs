...
public class PressForceHemViewModel : ViewModelBase
{
	...
	public void Init(IBendMachine bendMachine)
	{
		...
		ICrowningTable pressForceHemTable = _bendMachine.PressForceHemTable;
		if (pressForceHemTable != null)
		{
			Columns.AddRange(pressForceHemTable.Lengths.Select((double len) => new CrowningColumnViewModel
			{
				Length = len
			}));
			Items.AddRange(pressForceHemTable.Entries.Select((ICrowningTableEntry kvp) => new CrowningItemViewModel
			{
				MatGrpId = kvp.MaterialGroupId,
				Thickness = kvp.Thickness,
				Values = kvp.Values.ToArray()
	...
	public void Save()
	{
		...
		_bendMachine.PressForceHemTable.SetData(lengths, entries);
	}
}
