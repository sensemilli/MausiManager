...
public class LowerToolExtensionsViewModel : ProfilesAndPartsViewModel<LowerToolExtensionViewModel, LowerToolExtensionPieceViewModel>
{
    [CompilerGenerated]
    ...
    private readonly Contracts.Common.ITranslator _translator1;
    ...
    public override ObservableCollection<LowerToolExtensionViewModel> Profiles => base.ToolConfigModel.LowerToolsExtensions;

    public override ObservableCollection<LowerToolExtensionPieceViewModel> Pieces => base.ToolConfigModel.LowerToolExtentionPieces;

    public LowerToolExtensionsViewModel(IConfigProvider _configProvider, Contracts.Common.ITranslator translator, IUnitConverter _unitConverter, 
        IDrawToolProfiles _drawToolProfiles, IGlobalToolStorage _toolStorage, IModelFactory _modelFactory, 
        IMessageLogGlobal _messageLog, BystronicXMLImporter _bystronicXMLImporter, 
        DelemXMLImporter _delemXMLImporter, LvdXMLImporter _lvdXMLImporter,
        RadanMDBImporter _radanMDBImporter, RadanXMLImporter _radanXMLImporter, 
        TrumpfARVImporter _trumpfARVImporter, TrumpfMDBImporter _trumpfMDBImporter) : base(_configProvider, translator, _drawToolProfiles, _toolStorage, _modelFactory)
    {
        ...
        _translator1 = translator;
        VWidthTypes = new List<ComboboxEntry<VWidthTypes>>();
        
    }

    public override void DeleteProfile(LowerToolExtensionViewModel profile)
    {
        base.ToolConfigModel.DeleteLowerExtensionTool(profile);
    }

    public override void DeletePiece(LowerToolExtensionPieceViewModel piece)
    {
        base.ToolConfigModel.DeleteLowerToolExtentionPiece(piece);
    }

    public new LowerToolExtensionsViewModel Init(MachineToolsViewModel toolModel)
    {
        base.Init(toolModel);
        foreach (VWidthTypes value in System.Enum.GetValues(typeof(VWidthTypes)))
        {
            if (value != 0)
            {
                VWidthTypes.Add(new ComboboxEntry<VWidthTypes>(_translator1.Translate($"l_enum.VWidthTypes.{value}"), value));
            }
        }
        return this;
    }
    ...
    {
        LowerToolExtensionViewModel profile = new LowerToolExtensionViewModel(_003C_unitConverter_003EP, _003C_toolStorage_003EP, base.ToolConfigModel.CreateMultiTool(""), base.ToolConfigModel.CreateLowerToolGroup());
        AddProfile(profile);
    }
    ...
    public override void AddPiece()
    {
        ...
        {
            LowerToolExtensionPieceViewModel piece = base.ToolConfigModel.CreateLowerToolExtensionPiece(base.ToolConfigModel.SelectedToolList, base.SelectedProfile.MultiTool, "");
            AddPiece(piece);
        }
    }
