...
public class SphericalBlindHoleToCadConverter : MacroToCadConverterBase
{
	private const int ToolType = 78;

	public static bool AddCadGeoElements(SphericalBlindHole blindHole, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix, Cad2DDatabase db2D, IConfigProvider configProvider, Face topFace)
	{
		...
		if (num == 0)
		{
			...
			db2D.AddText(SphericalBlindHoleToCadConverter.GetCadTxtSide(vector3d.Dot(v3) - vector3d.Dot(blindHole.AnchorPoint), blindHole.CylinderRadius, v2));
		...
		{
			db2D.AddText(SphericalBlindHoleToCadConverter.GetCadTxt(blindHole.TotalDepth, blindHole.CylinderRadius, v, addOn));
	...
	private static CadTxtText GetCadTxt(double totalDepth, double radius, global::WiCAM.Pn4000.BendModel.Base.Vector3d middlePoint, string addOn)
	{
        const int toolType = 78;

        var text = string.Format(CultureInfo.InvariantCulture,
            "_TT_{0}_{1}_{2}_{3}",
            toolType,
            Math.Round(totalDepth, 4),
            Math.Round(2.0 * radius, 4),
            addOn);
	...
	{
		string text = string.Format(CultureInfo.InvariantCulture, "_HBS_{0}_{1}", Math.Round(distTop, 4), Math.Round(radius * 2.0, 4));
		return new CadTxtText
		{
			Text = text,
			Angle = 0.0,
	...
	public static SphericalBlindHoleXml GetXmlElement(SphericalBlindHole sphericalBlindHole, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix)
	{
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v = sphericalBlindHole.Orientation;
		...
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v2 = sphericalBlindHole.AnchorPoint;
		worldMatrix.TransformInPlace(ref v2);
		return new SphericalBlindHoleXml
		{
			TotalDepth = sphericalBlindHole.TotalDepth,
			Radius = sphericalBlindHole.CylinderRadius,
			ID = sphericalBlindHole.ID,
			Direction = direction,
