...
public class LouverToCadConverter : MacroToCadConverterBase
{
	private const int ToolType = 22;

	public static bool AddCadGeoElements(Louver louver, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix, Cad2DDatabase db2D, IConfigProvider configProvider)
	{
		Macro3DConfig macro3DConfig = configProvider.InjectOrCreate<Macro3DConfig>();
		double size = louver.Size0;
		double size2 = louver.Size1;
		double size3 = louver.Size2;
		bool flag;
		switch (MacroToCadConverterBase.GetOrientation(louver.Orientation, worldMatrix))
		{
		...
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v = louver.AnchorPoint;
		worldMatrix.TransformInPlace(ref v);
		int color = (flag ? macro3DConfig.MacroPosColorPn : macro3DConfig.MacroNegColorPn);
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v2 = louver.OrientationX;
		...
		foreach (Face face in louver.Faces)
		{
			foreach (FaceHalfEdge item in face.BoundaryEdgesCcw.Where((FaceHalfEdge x) => !louver.Faces.Contains(x.CounterEdge?.Face)))
			{
		...
		db2D.AddText(LouverToCadConverter.GetCadTxt(v, num, size, size2, size3, flag ? "+" : "-"));
		return true;
	}
	...
	public static LouverXml GetXmlElement(Louver louver, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix)
	{
		double size = louver.Size0;
		double size2 = louver.Size1;
		_ = louver.Size2;
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v = louver.Orientation;
		worldMatrix.TransformNormalInPlace(ref v);
		bool flag = Math.Round(v.Z, 5) > 0.0;
		global::WiCAM.Pn4000.BendModel.Base.Vector3d v2 = louver.AnchorPoint;
		worldMatrix.TransformInPlace(ref v2);
		double angle = 0.0;
		return new LouverXml
		{
			ID = louver.ID,
