...
public class EmbossedSquareRoundedToCadConverter : MacroToCadConverterBase
{
	private const int ToolType = 13;

	public static bool AddCadGeoElements(EmbossedSquareRounded embossed, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix, Cad2DDatabase db2D, IConfigProvider configProvider)
	{
		...
		global::WiCAM.Pn4000.BendModel.Base.Vector3d vector3d3 = normalized * (0.5 * embossed.Width - embossed.Radius);
		global::WiCAM.Pn4000.BendModel.Base.Vector3d vector3d4 = normalized * embossed.Radius;
		db2D.AddText(EmbossedSquareRoundedToCadConverter.GetCadTxt(anchorPoint, macro3DConfig.MacroEmbossedToolSuffix, embossed.Depth, depthSign, embossed.Width, embossed.Radius, num));
	...
	private static CadTxtText GetCadTxt(global::WiCAM.Pn4000.BendModel.Base.Vector3d middlePoint, string toolSuffix, double depth, string depthSign, double width, double radius, double angle)
	{
        ...
        var text = string.Format(CultureInfo.InvariantCulture,
            ...
            toolSuffix,
            Math.Round(width, 4),
            Math.Round(width, 4),  // Square has same width and height
            Math.Round(radius, 4),
            depthSign,
            Math.Round(depth, 4));
	...
	public static EmbossedSquareRoundedXml GetXmlElement(EmbossedSquareRounded embossed, global::WiCAM.Pn4000.BendModel.Base.Matrix4d worldMatrix)
	{
		...
		return new EmbossedSquareRoundedXml
		{
