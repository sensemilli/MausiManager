...
public class LowerHolderViewModel : ToolViewModelBase
{
	...
	public LowerHolderViewModel(IGlobals globals, IMainWindowDataProvider mainWindowDataProvider, IPnPathService pnPathService, IConfigProvider configProvider, IModelFactory modelFactory, IDrawToolProfiles drawToolProfiles)
	...
	public void Init(BendMachine bendMachine, ToolConfigModel toolConfigModel, IToolExpert tools = null)
	{
		...
		base.SelectedType = "LowerHolders";
	}
	...
	{
		this.HolderProfileView = (CollectionView)CollectionViewSource.GetDefaultView(base.ToolConfigModel.LowerHolderProfiles);
		this.HolderProfileView.Filter = HolderProfileFilter;
		this.HolderProfileView.Refresh();
		this.HolderPartView = (CollectionView)CollectionViewSource.GetDefaultView(base.ToolConfigModel.LowerHolderParts);
	...
	{
		Parallel.ForEach((IEnumerable<HolderProfileViewModel>)base.ToolConfigModel.LowerHolderProfiles, (Action<HolderProfileViewModel>)delegate(HolderProfileViewModel item)
		{
		...
		Parallel.ForEach((IEnumerable<HolderPartViewModel>)base.ToolConfigModel.LowerHolderParts, (Action<HolderPartViewModel>)delegate(HolderPartViewModel item)
		{
	...
	private bool HolderProfileFilter(object item)
	{
		...
		{
			return base.ToolConfigModel.LowerHolderParts.Any((HolderPartViewModel p) => p.HolderProfileID == ((HolderProfileViewModel)item).ID && ((!this.ShowAllParts && p.Amount > 0 && p.Implemented) || this.ShowAllParts));
		}
		return true;
	}
	...
	{
		int id = ((base.ToolConfigModel.LowerHolderProfiles.Count > 0) ? (base.ToolConfigModel.LowerHolderProfiles.Max((HolderProfileViewModel p) => p.ID) + 1) : 0);
		base.ToolConfigModel.LowerHolderProfiles.Add(new HolderProfileViewModel(new HolderProfile(id)));
		...
		this._changed = ChangedConfigType.LowerHolder;
	}
	...
	public void AddPart_Click()
	{
		...
		{
			int id = ((base.ToolConfigModel.LowerHolderParts.Count > 0) ? (base.ToolConfigModel.LowerHolderParts.Max((HolderPartViewModel p) => p.ID) + 1) : 0);
			base.ToolConfigModel.LowerHolderParts.Add(new HolderPartViewModel(new HolderPart(id, this.SelectedProfile.ID)));
	...
	public void CopyPart_Click()
	{
		if (this.SelectedPart != null)
		{
			...
			holderPart.ID = iD;
			base.ToolConfigModel.LowerHolderParts.Add(new HolderPartViewModel(holderPart));
			int num = this.HolderPartView.Count - 1;
			this.SelectedPart = ((num >= 0 && this.HolderPartView.Count > 0 && num < this.HolderPartView.Count) ? ((HolderPartViewModel)this.HolderPartView.GetItemAt(num)) : null);
			this._changed = ChangedConfigType.LowerHolder;
		}
	...
	public void CopyProfile_Click()
	{
		if (this.SelectedProfile != null)
		{
			...
			holderProfile.ID = iD;
			base.ToolConfigModel.LowerHolderProfiles.Add(new HolderProfileViewModel(holderProfile));
			int num = this.HolderProfileView.Count - 1;
			this.SelectedProfile = ((num >= 0 && this.HolderProfileView.Count > 0 && num < this.HolderProfileView.Count) ? ((HolderProfileViewModel)this.HolderProfileView.GetItemAt(num)) : null);
			this._changed = ChangedConfigType.LowerHolder;
		}
	...
	public void Delete_Click(object param)
	{
		...
		string text = param.GetType().ToString().Split('.')
			.Last();
		if (!(text == "LowerHolderProfileViewModel"))
		{
			if (text == "LowerHolderPartViewModel")
			{
	...
	{
		int num = base.ToolConfigModel.LowerHolderProfiles.IndexOf(this.SelectedProfiles.Last()) + 1 - this.SelectedProfiles.Count();
		...
		base.ToolConfigModel.LowerHolderProfiles = base.ToolConfigModel.LowerHolderProfiles.Except(list).ToObservableCollection();
		if (num >= base.ToolConfigModel.LowerHolderProfiles.Count)
		{
			num = base.ToolConfigModel.LowerHolderProfiles.Count - 1;
		}
		this.SetEditorEnableRules();
		if (num < 0)
		{
	...
	{
		int num = base.ToolConfigModel.LowerHolderParts.IndexOf(this.SelectedParts.Last()) + 1 - this.SelectedParts.Count();
		if (num > base.ToolConfigModel.LowerHolderParts.Count)
		{
		...
		base.ToolConfigModel.LowerHolderParts = base.ToolConfigModel.LowerHolderParts.Except(second).ToObservableCollection();
		this.SetEditorEnableRules();
		if (num < 0 && this.HolderPartView.Count > 0)
		{
			num = 0;
		}
		int num2 = ((this.HolderPartView.Count <= 0) ? (-1) : 0);
	...
	{
		base.ToolConfigModel.LowerHolderParts.Where((HolderPartViewModel p) => p.HolderProfileID == profileId).ToList().ForEach(delegate(HolderPartViewModel p)
		{
			base.ToolConfigModel.LowerHolderParts.Remove(p);
		});
	}
	...
	public void EditProfile_Click()
	{
		EditScreenViewModel dataContext = new EditScreenViewModel(closeAction: delegate(bool isOk, bool close)
		{
			if (isOk)
			{
				int index = base.ToolConfigModel.LowerHolderProfiles.IndexOf(this.SelectedProfile);
				base.ToolConfigModel.LowerHolderProfiles.Remove(this.SelectedProfile);
				base.ToolConfigModel.LowerHolderProfiles.Insert(index, ((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).SelectedItem as HolderProfileViewModel);
				this.SelectedProfile = base.ToolConfigModel.LowerHolderProfiles[index];
			}
			if (close)
			{
		...
		}, items: ((IEnumerable<HolderProfileViewModel>)base.ToolConfigModel.LowerHolderProfiles).Select((Func<HolderProfileViewModel, ToolItemViewModelBase>)((HolderProfileViewModel i) => i)), selectedItem: this.SelectedProfile, isUpper: false, toolConfigModel: base.ToolConfigModel, bendMachine: base.BendMachine, configProvider: this._configProvider);
	...
	public void EditPart_Click()
	{
		EditScreenViewModel dataContext = new EditScreenViewModel(closeAction: delegate(bool isOk, bool close)
		{
			if (isOk)
			{
				int index = base.ToolConfigModel.LowerHolderParts.IndexOf(this.SelectedPart);
				base.ToolConfigModel.LowerHolderParts.Remove(this.SelectedPart);
				base.ToolConfigModel.LowerHolderParts.Insert(index, ((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).SelectedItem as HolderPartViewModel);
				this.SelectedPart = base.ToolConfigModel.LowerHolderParts[index];
			}
			if (close)
			{
				((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).Dispose();
		...
		}, items: base.ToolConfigModel.LowerHolderParts.Where((HolderPartViewModel p) => p.HolderProfileID == this.SelectedProfile.ID).Select((Func<HolderPartViewModel, ToolItemViewModelBase>)((HolderPartViewModel i) => i)), selectedItem: this.SelectedPart, isUpper: false, toolConfigModel: base.ToolConfigModel, bendMachine: base.BendMachine, configProvider: this._configProvider);
	...
	public void Save()
	{
		base.BendMachine.Holder.LowerHolderProfiles = new ObservableCollection<HolderProfile>();
		foreach (HolderProfileViewModel lowerHolderProfile in base.ToolConfigModel.LowerHolderProfiles)
		{
			base.BendMachine.Holder.LowerHolderProfiles.Add(lowerHolderProfile.HolderProfile);
		}
		base.BendMachine.Holder.LowerHolderParts = new ObservableCollection<HolderPart>();
		foreach (HolderPartViewModel lowerHolderPart in base.ToolConfigModel.LowerHolderParts)
		{
			base.BendMachine.Holder.LowerHolderParts.Add(lowerHolderPart.HolderPart);
		}
		if (base.ToolConfigModel.LowerHolderProfiles.Any((HolderProfileViewModel p) => p.IsChanged))
		{
			this._changed = ChangedConfigType.LowerHolder;
		}
		if (base.ToolConfigModel.LowerHolderParts.Any((HolderPartViewModel p) => p.IsChanged))
		{
			this._changed = ChangedConfigType.LowerHolder;
		}
