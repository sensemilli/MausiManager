...
public class LowerAdapterViewModel : ToolViewModelBase
{
	...
	public LowerAdapterViewModel(IGlobals globals, IMainWindowDataProvider mainWindowDataProvider, IPnPathService pnPathService, IConfigProvider configProvider, IModelFactory modelFactory, IDrawToolProfiles drawToolProfiles)
	...
	{
		this.AdapterProfileView = (CollectionView)CollectionViewSource.GetDefaultView(base.ToolConfigModel.LowerAdapterProfiles);
		this.AdapterProfileView.Filter = AdapterProfileFilter;
		this.AdapterProfileView.Refresh();
		this.AdapterPartView = (CollectionView)CollectionViewSource.GetDefaultView(base.ToolConfigModel.LowerAdapterParts);
	...
	{
		Parallel.ForEach((IEnumerable<AdapterProfileViewModel>)base.ToolConfigModel.LowerAdapterProfiles, (Action<AdapterProfileViewModel>)delegate(AdapterProfileViewModel item)
		{
		...
		Parallel.ForEach((IEnumerable<AdapterPartViewModel>)base.ToolConfigModel.LowerAdapterParts, (Action<AdapterPartViewModel>)delegate(AdapterPartViewModel item)
		{
	...
	private bool AdapterProfileFilter(object item)
	{
		...
		{
			return base.ToolConfigModel.LowerAdapterParts.Any((AdapterPartViewModel p) => p.AdapterProfileID == ((AdapterProfileViewModel)item).ID && ((!this.ShowAllParts && p.Amount > 0 && p.Implemented) || this.ShowAllParts));
		}
		return true;
	}
	...
	{
		int id = ((base.ToolConfigModel.LowerAdapterProfiles.Count > 0) ? (base.ToolConfigModel.LowerAdapterProfiles.Max((AdapterProfileViewModel p) => p.ID) + 1) : 0);
		base.ToolConfigModel.LowerAdapterProfiles.Add(new AdapterProfileViewModel(new AdapterProfile(id)));
		this.SelectedProfile = base.ToolConfigModel.LowerAdapterProfiles.LastOrDefault();
		this._changed = ChangedConfigType.LowerAdapter;
	}
	...
	public void AddPart_Click()
	{
		...
		{
			int id = ((base.ToolConfigModel.LowerAdapterParts.Count > 0) ? (base.ToolConfigModel.LowerAdapterParts.Max((AdapterPartViewModel p) => p.ID) + 1) : 0);
			base.ToolConfigModel.LowerAdapterParts.Add(new AdapterPartViewModel(new AdapterPart(id, this.SelectedProfile.ID)));
	...
	public void CopyPart_Click()
	{
		if (this.SelectedPart != null)
		{
			...
			adapterPart.ID = iD;
			base.ToolConfigModel.LowerAdapterParts.Add(new AdapterPartViewModel(adapterPart));
			int num = this.AdapterPartView.Count - 1;
			this.SelectedPart = ((num >= 0 && this.AdapterPartView.Count > 0 && num < this.AdapterPartView.Count) ? ((AdapterPartViewModel)this.AdapterPartView.GetItemAt(num)) : null);
			this._changed = ChangedConfigType.LowerAdapter;
		}
	...
	public void CopyProfile_Click()
	{
		if (this.SelectedProfile != null)
		{
			...
			base.ToolConfigModel.LowerAdapterProfiles.Add(new AdapterProfileViewModel(adapterProfile));
	...
	{
		int num = base.ToolConfigModel.LowerAdapterProfiles.IndexOf(this.SelectedProfiles.Last()) + 1 - this.SelectedProfiles.Count();
		...
		base.ToolConfigModel.LowerAdapterProfiles = base.ToolConfigModel.LowerAdapterProfiles.Except(list).ToObservableCollection();
		if (num >= base.ToolConfigModel.LowerAdapterProfiles.Count)
		{
			num = base.ToolConfigModel.LowerAdapterProfiles.Count - 1;
		}
		this.SetEditorEnableRules();
		if (num < 0)
		{
	...
	{
		int num = base.ToolConfigModel.LowerAdapterParts.IndexOf(this.SelectedParts.Last()) + 1 - this.SelectedParts.Count();
		if (num > base.ToolConfigModel.LowerAdapterParts.Count)
		{
		...
		base.ToolConfigModel.LowerAdapterParts = base.ToolConfigModel.LowerAdapterParts.Except(second).ToObservableCollection();
		this.SetEditorEnableRules();
	...
	{
		base.ToolConfigModel.LowerAdapterParts.Where((AdapterPartViewModel p) => p.AdapterProfileID == profileId).ToList().ForEach(delegate(AdapterPartViewModel p)
		{
			base.ToolConfigModel.LowerAdapterParts.Remove(p);
		});
	}
	...
	public void EditProfile_Click()
	{
		EditScreenViewModel dataContext = new EditScreenViewModel(closeAction: delegate(bool isOk, bool close)
		{
			if (isOk)
			{
				int index = base.ToolConfigModel.LowerAdapterProfiles.IndexOf(this.SelectedProfile);
				base.ToolConfigModel.LowerAdapterProfiles.Remove(this.SelectedProfile);
				base.ToolConfigModel.LowerAdapterProfiles.Insert(index, ((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).SelectedItem as AdapterProfileViewModel);
				this.SelectedProfile = base.ToolConfigModel.LowerAdapterProfiles[index];
			}
			if (close)
			{
		...
		}, items: ((IEnumerable<AdapterProfileViewModel>)base.ToolConfigModel.LowerAdapterProfiles).Select((Func<AdapterProfileViewModel, ToolItemViewModelBase>)((AdapterProfileViewModel i) => i)), selectedItem: this.SelectedProfile, isUpper: false, toolConfigModel: base.ToolConfigModel, bendMachine: base.BendMachine, configProvider: this._configProvider);
	...
	public void EditPart_Click()
	{
		EditScreenViewModel dataContext = new EditScreenViewModel(closeAction: delegate(bool isOk, bool close)
		{
			if (isOk)
			{
				int index = base.ToolConfigModel.LowerAdapterParts.IndexOf(this.SelectedPart);
				base.ToolConfigModel.LowerAdapterParts.Remove(this.SelectedPart);
				base.ToolConfigModel.LowerAdapterParts.Insert(index, ((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).SelectedItem as AdapterPartViewModel);
				this.SelectedPart = base.ToolConfigModel.LowerAdapterParts[index];
			}
			if (close)
			{
				((this.EditScreen as EditScreenView).DataContext as EditScreenViewModel).Dispose();
		...
		}, items: base.ToolConfigModel.LowerAdapterParts.Where((AdapterPartViewModel p) => p.AdapterProfileID == this.SelectedProfile.ID).Select((Func<AdapterPartViewModel, ToolItemViewModelBase>)((AdapterPartViewModel i) => i)), selectedItem: this.SelectedPart, isUpper: false, toolConfigModel: base.ToolConfigModel, bendMachine: base.BendMachine, configProvider: this._configProvider);
	...
	public void Save()
	{
		base.BendMachine.Adapter.LowerAdapterProfiles = new ObservableCollection<AdapterProfile>();
		foreach (AdapterProfileViewModel lowerAdapterProfile in base.ToolConfigModel.LowerAdapterProfiles)
		{
			base.BendMachine.Adapter.LowerAdapterProfiles.Add(lowerAdapterProfile.AdapterProfile);
		}
		base.BendMachine.Adapter.LowerAdapterParts = new ObservableCollection<AdapterPart>();
		foreach (AdapterPartViewModel lowerAdapterPart in base.ToolConfigModel.LowerAdapterParts)
		{
			base.BendMachine.Adapter.LowerAdapterParts.Add(lowerAdapterPart.AdapterPart);
		}
		if (base.ToolConfigModel.LowerAdapterProfiles.Any((AdapterProfileViewModel p) => p.IsChanged))
		{
			this._changed = ChangedConfigType.LowerAdapter;
		}
		if (base.ToolConfigModel.LowerAdapterParts.Any((AdapterPartViewModel p) => p.IsChanged))
		{
			this._changed = ChangedConfigType.LowerAdapter;
		}
